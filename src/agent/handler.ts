...
async function processAgentMessage(params: {
    ...
    const identity = resolveInboundAgentIdentity({
        agent,
        messageToUserName: extractToUser(msg),
    });
    if (identity.kind === "reject") {
        error?.(`[wecom-agent] rejected callback identity: ${identity.reason}`);
        return;
    }
    if (identity.kind === "upstream" && isGroup) {
        error?.(
            `[wecom-agent] rejected upstream group callback corpId=${identity.upstream.corpId} chatId=${peerId}`,
        );
        return;
    }

    const upstream = identity.kind === "upstream" ? identity.upstream : undefined;
    const agentTarget = upstream
        ? buildUpstreamAgentTarget({
            accountId: agent.accountId,
            corpId: upstream.corpId,
            userId: fromUser,
        })
        : `wecom-agent:${fromUser}`;
    const sourceId = upstream ? agentTarget : fromUser;
    const routePeerId = isGroup ? peerId : sourceId;
    const sendReplyText = (text: string) => upstream
        ? sendUpstreamText({ primaryAgent: agent, upstream, toUser: fromUser, text })
        : sendText({ agent, toUser: fromUser, chatId: undefined, text });
    ...
    if (["image", "voice", "video", "file"].includes(msgType)) {
        ...
        if (mediaId) {
            try {
                ...
                const { buffer, contentType, filename: headerFileName } = upstream
                    ? await downloadUpstreamMedia({
                        primaryAgent: agent,
                        upstream,
                        mediaId,
    ...
    const route = core.channel.routing.resolveAgentRoute({
        ...
        channel: "wecom",
        accountId: agent.accountId,
        peer: { kind: isGroup ? "group" : "direct", id: routePeerId },
    });
    ...
    const mediaLocalRoots = getAgentScopedMediaLocalRoots(config, route.agentId);
    // ===== 动态 Agent 路由处理结束 =====

    // 构建上下文
    const fromLabel = isGroup ? `group:${peerId}` : upstream ? agentTarget : `user:${fromUser}`;
    ...
    if (authz.shouldComputeAuth && authz.commandAuthorized !== true) {
        const prompt = buildWecomUnauthorizedCommandPrompt({
            ...
            scope: "agent",
            accountId: agent.accountId,
            multiAccount: Boolean(config.channels?.wecom?.accounts),
        });
        try {
            await sendReplyText(prompt);
    ...
    const ctxPayload = core.channel.reply.finalizeInboundContext({
        ...
        From: isGroup ? `wecom:group:${peerId}` : upstream ? agentTarget : `wecom:${fromUser}`,
        ...
        dispatcherOptions: {
            deliver: async (payload: { text?: string; mediaUrls?: string[]; mediaUrl?: string }, info: { kind: string }) => {
                ...
                for (const mediaPath of mediaUrls) {
                    try {
                        const media = await resolveMediaFile(mediaPath, mediaLocalRoots);
                        const detectedType = detectWeComMediaType(media.contentType);
                        const sizeCheck = applyFileSizeLimits(
                            media.buffer.length,
                            detectedType,
                            media.contentType,
                        );
                        ...
                        const mediaType = sizeCheck.finalType;

                        log?.(`[wecom-agent] uploading media: ${media.fileName} (${mediaType}, ${media.contentType}, ${media.buffer.length} bytes)`);

                        const mediaId = upstream
                            ? await uploadUpstreamMedia({
                                primaryAgent: agent,
                                upstream,
                                type: mediaType,
                                buffer: media.buffer,
                        ...
                        if (upstream) {
                            await sendUpstreamMedia({
                                ...
                                mediaId,
                                mediaType,
                                ...(mediaType === "video" ? { title: media.fileName, description: "" } : {}),
                            });
                        } else {
                        ...
                        log?.(`[wecom-agent] media sent (${info.kind}) to ${fromUser}: ${media.fileName} (${mediaType})`);
                    } catch (err: unknown) {
                        const message = err instanceof Error ? `${err.message}${err.cause ? ` (cause: ${String(err.cause)})` : ""}` : String(err);
                        error?.(`[wecom-agent] media send failed: ${mediaPath}: ${message}`);
                        // 降级：发文本通知用户
                        try {
                            await sendReplyText(`⚠️ 文件发送失败: ${mediaPath.split("/").pop() || mediaPath}\n${message}`);
                        } catch { /* ignore */ }
                    }
                }
